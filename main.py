...
# 引入一个Constant作为模型初始化的embedding矩阵
...
class BiDAF:
    ...
    def __init__(
            self, clen, qlen, emb_size,embedding_matrix,
            max_features=5000,
    ...
    def build_model(self):
        ...
        embedding_layer = tf.keras.layers.Embedding(self.embedding_matrix.shape[0],
                                                     self.emb_size,
                                                     embeddings_initializer=Constant(self.embedding_matrix),## 这里将'uniform'改为了常矩阵embedding_matrix，其行数为cqa的所有词的总数，
        input_length=1000)
...
if __name__ == '__main__':
    ...
    bidaf = BiDAF(
        ...
        embedding_matrix = ds.WordEmbedding(),
    )
    bidaf.build_model()
    '''
    bidaf.model.fit(
